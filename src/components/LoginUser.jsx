...
export default function LoginUser(props) {
    ...
    const handleAuth = async (event) => {
        ...
        try {
            ...
            // Extract token and handle it as needed
            const { token, refreshToken } = response.data;
            
            // Save tokens or handle authentication
            localStorage.setItem('token', token); // Example of storing token
            localStorage.setItem('refreshToken', refreshToken); // Example of storing refresh token
    ...
    return (
        <div className="w-full max-w-xs bg-gradient-to-b from-gray-600 to-gray-700 p-8 rounded-lg shadow-2xl">
            ...
            <form className="space-y-4" onSubmit={handleAuth}>
                ...
                <h1 className="text-red-500 mt-2">{feedback}</h1>
            </form>
        </div>
    );
}
